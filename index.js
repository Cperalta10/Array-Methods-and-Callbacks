...
//onsole.log(finals2014[0]["Home Team Goals"]);
...
function getYears(array, getFinalsCB) {
  let allFinals = getFinals(array);
  let years = allFinals.map((item) => item.Year);
  return years;
}
...
function getAverageGoals(data) {
  let averageResult = data.reduce(function (acc, item) {
    return acc + item["Home Team Goals"] + item["Away Team Goals"];
  }, 0);
  return (averageResult / data.length).toFixed(2);
}
/// 🥅 STRETCH 🥅 ///
